...
const statusMap = {
  recruiting: { label: '모집중', bg: '#e3f2fd', color: '#1565c0' },
  confirmed: { label: '확정', bg: '#e8f5e9', color: '#2e7d32' },
  past: { label: '종료', bg: '#f5f5f5', color: '#757575' },
};
...
export default function Meetups() {
  ...
  const handleToggleHide = async (meetupId, currentHidden) => {
    try {
      await updateDoc(doc(db, 'meetups', meetupId), { hidden: !currentHidden });
      setMeetups((prev) =>
        prev.map((m) => (m.id === meetupId ? { ...m, hidden: !currentHidden } : m))
      );
    } catch (err) {
  ...
  const filteredMeetups = meetups.filter((meetup) => {
    if (statusFilter !== 'all' && meetup.status !== statusFilter) return false;
    if (search) {
      const s = search.toLowerCase();
      const title = (meetup.title || '').toLowerCase();
      const host = (meetup.hostName || '').toLowerCase();
      if (!title.includes(s) && !host.includes(s)) return false;
    }
    return true;
  });

  const totalPages = Math.max(1, Math.ceil(filteredMeetups.length / PAGE_SIZE));
  const paginatedMeetups = filteredMeetups.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  ...
  const getStatusBadge = (status) => {
    const info = statusMap[status] || statusMap.recruiting;
    return (
      <span
        style={{
          display: 'inline-block',
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>모임 관리</h1>
      ...
      <table style={styles.table}>
        <thead>
          <tr>
            ...
            <th style={styles.th}>주최자</th>
            <th style={styles.th}>일시</th>
            <th style={{ ...styles.th, textAlign: 'center' }}>참석자</th>
            ...
            paginatedMeetups.map((meetup) => (
              <tr key={meetup.id}>
                <td style={styles.td}>
                  {meetup.title || '-'}
                  {meetup.hidden && (
                    <span style={{ ...styles.badgeHidden, marginLeft: 6 }}>숨김</span>
                  )}
                </td>
                <td style={styles.td}>{meetup.hostName || '-'}</td>
                <td style={styles.td}>{formatDate(meetup.date)}</td>
                <td style={{ ...styles.td, textAlign: 'center' }}>
                  {Array.isArray(meetup.attendees) ? meetup.attendees.length : 0}명
                </td>
                <td style={styles.td}>{meetup.location || '-'}</td>
                <td style={{ ...styles.td, textAlign: 'center' }}>
                ...
                <td style={styles.td}>
                  <div style={styles.actions}>
                    <button
                      ...
                      onClick={() => handleToggleHide(meetup.id, meetup.hidden)}
                    >
                      {meetup.hidden ? '공개' : '숨김'}
                    </button>
                    <button
